...
import { PaginatedResponse, SharedData, TaskData } from '@/types';

export const useTaskTable = ({ data }: { data: PaginatedResponse<TaskData> }) => {
    ...
    const deleteBulk = (ids: number[]) => {
        ...
        // Using Inertia router delete with IDs in query
        router.delete(`/tasks`, {
            data: { ids },
            preserveScroll: true,
